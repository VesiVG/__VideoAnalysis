    ...
    partial class Program
    {
        ...
        static void Main(string[] args)
        {
            ...
            if (mode == RunMode.Opengl)
            {
                ...
                using (OpenTK.Toolkit.Init(new OpenTK.ToolkitOptions() { Backend = OpenTK.PlatformBackend.PreferNative, EnableHighResolution = false }))
                {
                    RunMe();
                }
            }
        ...
        public static EventWaitHandle ready;
        ...
        private static unsafe void Display_task()
        {
            System.Threading.Tasks.Task.Factory.StartNew((Action)(() =>
            {
                while (bgr_work_need_stop == false)
                {
                    ...
                    if (bgr_work_need_stop == false)
                    {
                        ...
                        if (pool != null)
                        {
                            ...
                            if (mode == RunMode.Opengl) Process_GL();
                            else if (mode == RunMode.Winforms) Process_Winforms();
        ...
        private static unsafe void Process_GL()
        {
            ...
            if (ca == 0)
            {
                ...
                sourcePixelFormat = FFWrapper.Tool.MatchPer((AVPixelFormat)cfa.pFrame->format);
                align = FFWrapper.Decoder.alignment;
                if (conv == null)
                {
                    var _szz = sourceSize;
                    if ((pic!=null && pic.glmode == GLW.GlMode.Simple_FixedFunctionImmediate) )
                        conv = new FFWrapper.Converter_video(sourceSize, sourcePixelFormat, _szz, AVPixelFormat.AV_PIX_FMT_BGR24, cfa.pFrame->color_range == AVColorRange.AVCOL_RANGE_JPEG ? 1 : 0, align);
                    else if (sourcePixelFormat != AVPixelFormat.AV_PIX_FMT_YUV420P)
                        conv = new FFWrapper.Converter_video(sourceSize, sourcePixelFormat, _szz, AVPixelFormat.AV_PIX_FMT_BGR24, cfa.pFrame->color_range == AVColorRange.AVCOL_RANGE_JPEG ? 1 : 0, align);
                    if (!_one)
                    {
                        _one = true;
                        if (conv != null) FFWrapper.logger?.log_m(1, 1, $"Using SwS conversion to  {conv._destinationPixelFormat}.\n");
                        else FFWrapper.logger?.log_m(1, 1, $"Not using frame conversion.\n");
                    }
                }
                ...
                if (conv != null)
                {
                    ...
                    var cpp3 = new FFWrapper.MediaFrame(cpp2, cfa.Id, conv._destinationPixelFormat);
...
*/
        ...
        public unsafe static void UpdateImg(FFWrapper.MediaFrame ff, System.Drawing.Image img)
        {
			 ...
			 */
            ffmpeg.av_image_copy_plane(s0, stride, ff.pFrame->data[0], ff.pFrame->linesize[0], width * 3, height);
