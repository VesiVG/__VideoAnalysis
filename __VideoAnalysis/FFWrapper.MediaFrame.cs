    ...
    public unsafe partial class FFWrapper
    {
        ...
        public class Pool : IDisposable
        {
            ...
            public struct Content : IDisposable
            {
                ...
                public bool IsEmpty;
                public Content(MediaFrame nv = null, int ii = -1, bool iisemp=true)
                {
            ...
            public void Add(AVFrame* val)
            {
                if (_pool.Count == MaxCapacity)
                {
                    RemoveLast();
                }
                lock (_locker) _pool.Add(new Content(new MediaFrame(val)));
            }
            public int GetNext(out MediaFrame _frame)
            {
                ...
                lock (_locker)
                {
                    _frame.BaitAndSwitch(_pool[0].Frame.pFrame);
                    _pool[0].Dispose();
                    _pool.RemoveAt(0);
                }
                return 0;
